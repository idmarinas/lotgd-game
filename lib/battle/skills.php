...
function rolldamage()
{
	...
	if ($badguy['creaturehealth']>0 && $session['user']['hitpoints']>0)
    {
		...
		if ($options['type'] != 'pvp') $adjustedcreaturedefense = ($creaturedefmod * $badguy['creaturedefense'] / ($adjustment * $adjustment));
		...
		*/
		...
		while(!isset($creaturedmg) || !isset($selfdmg) || $creaturedmg == 0 && $selfdmg == 0)
        {
			...
			*/
            ...
            if ($powerattack != 0 && $options['type'] != 'pvp' && e_rand(1, $powerattack) == 1) $catkroll *= $powerattackmulti;
