...
function addcommentary() {
	...
	if ($remove>0) {
		//remove a sentence from the [DEL] at the side of each post (when you're entitled to see it of course
...
function injectsystemcomment($section,$comment) {
	//function lets gamemasters put in comments without a user association...be careful, it is not trackable who posted it (intentionally)
...
function injectrawcomment($section, $author, $comment) {
	//inject a raw coment directly into the commentary table and invalidate caches
...
function commentdisplay($intro, $section, $message="Interject your own commentary?",$limit=10,$talkline="says",$schema=false) {
	...
	//display the section and submit the same parameter set
...
function talkform($section,$talkline,$limit=10,$schema=false){
	...
	previewfield("insertcommentary", $session['user']['name'], $talkline, true, array("size"=>getsetting('chatlinelength',40), "maxlength"=>getsetting('maxchars',200)-$tll));
