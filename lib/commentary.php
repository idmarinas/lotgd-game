...
function removecommentary($cid, $reason, $mod)
{
	...
	if (isset($row['info']['restored'])) unset($row['info']['restored']);
...
function commentcleanup($comment)
{
	...
	if ($italics && $italics%2) { $comment .= '`i'; }

	if (substr($comment, -2) != '`0') $comment . '`0';//-- For close color tag
...
function injectcommentary($section, $talkline, $comment)
{
	...
	if ($session['user']['clanid'] && $session['user']['clanrank'])
	{
		...
		if (is_array($clanrow) && ! empty($clanrow)) { $info = array_merge($info, $clanrow); }
		else
		{
	...
	else
	{
		...
		{
			if ($commentbuffer[0]['comment'] == $commentary) $doublepost = true;
		}
		else
		{
			$select = DB::select('commentary');
			...
			if (is_array($row))
			{
				if ($row['comment'] == $commentary) $doublepost = true;
...
{
	if (is_array($info)) { $sqlinfo = serialize($info); }
	else { $sqlinfo = serialize([$info]); }
 ...
 * @param string $talkline
 * @param boolean $schema
 * @param boolean $skipfooter
 * @param false|string $customsql
 * @param boolean $skiprecentupdate
 * @param boolean $overridemod
 *
 * @return void
 */
function viewcommentary($section, $message = 'Interject your own commentary?', $limit = 25, $talkline = 'says', $schema = false, $skipfooter = false, $customsql = false, $skiprecentupdate = false, $overridemod = false)
{
	...
	if (! isset($returnlink) || ! $returnlink) $returnlink = urlencode($REQUEST_URI);
...
function preparecommentaryblock($section, $message = 'Interject your own commentary?', $limit = 25, $talkline = 'says', $schema = false, $skipfooter = false, $customsql = false, $skiprecentupdate = false, $overridemod = false, $returnlink = false)
{
	...
	{
		if (substr($section, strlen($section)-4,strlen($section)) != '_aux') { $chatloc = $section; }
		else { $chatloc = substr($section, 0, -4); }
	...
	if (($moderating && $rowcount) || !$moderating)
	{
		...
		if (!isset($session['user']['prefs']['commentary_recentline'])) { $session['user']['prefs']['commentary_recentline'] = 1; }

		$ret .= '<div class="ui commentary list">';

		if (! $session['user']['prefs']['commentary_reverse']) { $commentbuffer = array_reverse($commentbuffer); }
		if (! isset($session['recentcomments'])) $session['recentcomments'] = 0;

		foreach($commentbuffer as $i => $comment)
		{
			if ($comment['postdate'] > $session['recentcomments'] && $session['user']['prefs']['commentary_recentline']) $new = 1;
			else $new = 0;
...
function getcommentary($section, $limit = 25, $talkline, $customsql = false, $showmodlink = false, $returnlink = false)
{
	...
	if (! $returnlink) $returnlink = urlencode($REQUEST_URI);
	...
	foreach ($commentbuffer as $i => $comment)
	{
		if ((! isset($commentbuffer[$i]['info']['hidecomment']) || ! $commentbuffer[$i]['info']['hidecomment']) || $showmodlink)
		{
			...
			if (substr($row['comment'], 0, 1) == ':' || substr($row['comment'], 0, 3) == '/me')
			{
				...
				if (substr($row['comment'], 0, 3) == '/me') { $row['comment'] = substr($row['comment'], 3); }
				elseif (substr($row['comment'], 0, 2) == '::') { $row['comment'] = substr($row['comment'], 2); }
			...
			if (! $row['skiptalkline'])
			{
				$thiscomment = sprintf('%s `3%s, "%s%s`3"`0 %s',
					$thiscomment,
					$talkline,
					(!isset($row['info']['talkcolour']) || $row['info']['talkcolour'] === false) ? '`#' : '`'.$row['info']['talkcolour'],
...
function preparecommentaryline($line)
{
	...
	if (! isset($line['gamecomment']) || ! $line['gamecomment'])
	{
		...
		$online = $icons['online'];
		unset($icons['online']);
		if (isset($online['icon'])) $finaloutput .= sprintf('<img src="%s" title="%s" alt="%s"> ', $online['icon'], $online['mouseover'], $online['mouseover']);
...
function buildcommentarylink($append, $returnlink = false)
{
	...
	$jump = false;
	if (isset($session['user']['prefs']['nojump']) && $session['user']['prefs']['nojump'] == true) { $jump = true; }

	if (!$returnlink) { $nlink = comscroll_sanitize($REQUEST_URI); }
	else
	{
...
function talkform($section, $talkline, $limit = 10, $schema = false)
{
	...
	if ($section=="X")
	{
		...
		while (list($key,$val)=each($sections)) { output_notl("<option value='$key'>$val</option>",true); }
