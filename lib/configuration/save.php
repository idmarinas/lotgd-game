...
if ($postSettings['villagename']
    && $postSettings['villagename'] != LotgdSetting::getSetting(
	'villagename',
	LOCATION_FIELDS
  )) {
	...
	//-- Moving from, to
	$charRepository->movingPlayersToLocation(
	  LotgdSetting::getSetting('villagename', LOCATION_FIELDS),
	  $postSettings['villagename']
	);
...
if ($postSettings['innname'] && $postSettings['innname'] != LotgdSetting::getSetting('innname', LOCATION_INN)) {
	//-- Moving from, to
	$charRepository->movingPlayersToLocation(
	  LotgdSetting::getSetting('innname', LOCATION_INN),
	  $postSettings['innname']
	);
...
foreach ($postSettings as $key => $val) {
	...
	if (!isset($current[$key]) || ($val != $current[$key])) {
		...
		$flashMessages .= LotgdTranslator::t(
		  'flash.message.default.save.change.setting',
		  ['key' => $key, 'oldValue' => $old[$key], 'newValue' => $val],
		  $textDomain
		);
