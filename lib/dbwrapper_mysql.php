...
function db_query($sql, $die=true){
	...
	if (!$r && $die === true) {
	 	if (defined("IS_INSTALLER") && IS_INSTALLER){
			...
			if ($session['user']['superuser'] & SU_DEVELOPER || 1){
				...
				die("A most bogus error has occurred.  I apologise, but the page you were trying to access is broken.  Please use your browser's back button and try again. Additionally, report this via petition to somebody from staff with the precise location and what you did. <br/><br/>Thanks.");
			}
		}
	}
	...
	if (!isset($dbinfo['querytime']))  $dbinfo['querytime'] = $endtime-$starttime;
		else $dbinfo['querytime'] += $endtime-$starttime;
	return $r;
}
