...
if ($op==""){
	...
	if (DB::num_rows($result)>0){
		...
		for ($i=0;$i<DB::num_rows($result);$i++){
			...
			output_notl("<tr>",true);
            output_notl("<td class='collapsing'><input id='checkbox$i' type='checkbox' name='msg[]' value='{$row['messageid']}'><i class='fa fa-fw ".($row['seen']?"fa-eye":"fa-envelope-o")."'></i><img src='images/".($row['seen']?"old":"new")."scroll.GIF' width='16' height='16' alt='".($row['seen']?"Old":"New")."'></td>",true);
