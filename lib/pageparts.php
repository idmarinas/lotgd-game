...
<input type="image" src="images/paypal2.gif" border="0" name="submit" alt="Donate to the Site">
	...
	if ($session['user']['superuser'] & SU_EDIT_PETITIONS){
		...
		if ($session['user']['superuser'] & SU_EDIT_USERS){
			$p = "<i class='fa fa-fw fa-angle-double-right'></i> <a href='user.php'>$ued</a> <i class='fa fa-fw fa-angle-double-right'></i> <a href='viewpetition.php'>$pet</a> ";
			addnav("", "user.php");
			addnav("", "viewpetition.php");
		} else {
			$p = "<i class='fa fa-fw fa-angle-double-right'></i> <a href='viewpetition.php'>$pet</a>";
			addnav("", "viewpetition.php");
		}
	...
	if ($session['loggedin']){
		...
		*/
        ...
        if ($u['alive']) {
			...
			addcharstat("Hitpoints", $u['hitpoints'].check_temp_stat("hitpoints",1)."`0/".$u['maxhitpoints'].check_temp_stat("maxhitpoints",1)." `\$<span title='".explained_get_player_attack()."'>(?)</span>`0".check_temp_stat("attack",1));
