...
function lotgd_showform ($layout, $row, $nosave = false, $keypref = false, $print = true, ?callable $callback = null)
{
	...
	$content = '<div class="shadow overflow-hidden border border-lotgd-200 sm:rounded-lg" data-controller="tabs" data-tabs-active-tab="italic uppercase active">'
	           . $content;
	$save = 'Save';
	...
	switch ($title) {
		...
		case 'radio':
			...
			foreach ($info as $k => $v) {
				...
				next($info);
				$optdis = $v;
				$select .= ("<div class='ui radio checkbox'><input type='radio' name='{$keyout}' value='{$optval}'"
				            . ($row[$key] == $optval ? ' checked' : '')
				            . '><label>'
				            . ("{$optdis}")
				            . '</label></div>');
			}
		...
		case 'bool':
			...
			return '<div class="relative">
                ...
                <input type="checkbox" value="1" name="' . $keyout . '" class="unstyle hidden" ' . (1 == $value ?
				' checked' : '') . '>
                <!-- path -->
