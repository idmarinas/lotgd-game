...
if ('' == httpget('subop'))
{
    ...
    if (1 == $row['loggedin'] && $row['laston'] > date('Y-m-d H:i:s', strtotime('-'.getsetting('LOGINTIMEOUT', 900).' seconds')))
    {
        rawoutput("<br><input type='submit' class='ui red button' value='$save'>");
        output_notl('`$');
    ...
    {
        $row['dragonpointssummary'] = array_count_values(($row['dragonpoints'] > '' ? unserialize($row['dragonpoints']) : []));
    }
    else
    {
