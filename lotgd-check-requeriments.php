...
<body>
	...
	<div id="main">
		<table>
			...
			<tr>
				<?php
				foreach ($lotgdVersions as $version): ?>
		...
		<p>The table indicates for each version if the requirements for installing Legend of the Green Dragon on this
		   server are met.</p>
		<p>The version indicated is the version in which the requirements are changed, so if you pass the tests for one
		   version you will also pass them for the next, until otherwise indicated.</p>
		<?php
		if ($hasFail): ?>
		...
		<section>
			<table>
				...
				<tr>
					...
					foreach ($lotgdVersions as $version): ?>
						<?php
						$data = $results[$version]['details']['php_version'] ?>
				...
				<tr>
					...
					foreach ($lotgdVersions as $version): ?>
						<?php
						$data = $results[$version]['details']['memory_limit'] ?>
						<td class="<?= $data ['isPassed'] ? 'success' : 'fail' ?>">
							<?php
							if ($data ['isPassed']): ?>
								Success
				...
				<tr>
					...
					foreach ($lotgdVersions as $version): ?>
						<?php
						$data = $results[$version]['details']['execution_time'] ?>
						<td class="<?= $data ['isPassed'] ? 'success' : 'fail' ?>">
							<?php
							if ($data ['isPassed']): ?>
								Success
				...
				<?php
				foreach ($extensionsFullList as $ext => $name): ?>
					<tr>
						<th scope="col" class="smaller"><?= $name ?></th>
						<?php
						foreach ($lotgdVersions as $version): ?>
							<?php
							if (isset($results[$version]['details']['extensions']['details'][$ext])): ?>
								<?php
		...
		<section>
			<table>
				...
				<tr>
					...
					foreach ($lotgdVersions as $version): ?>
						<?php
						$data = $results[$version]['details']['mysql_version'] ?>
						<?php
						if ($data['isError']): ?>
							<?php
							$hasError = true ?>
