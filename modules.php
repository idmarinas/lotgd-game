...
ksort($seencats);
addnav([' ?Uninstalled - (%s modules)', $ucount], 'modules.php');
reset($seencats);

foreach ($seencats as $cat => $count)
{
    addnav([' ?%s - (%s modules)', $cat, $count], "modules.php?cat=$cat");
}
...
if ('' == $op)
{
    if ($cat)
    {
        ...
        rawoutput("<thead><tr><th>&nbsp;</th><th>$ops</th><th><a href='modules.php?cat=$cat&sortby=active&order=".('active' == $sortby ? ! $order : 1)."'>$status</a></th><th><a href='modules.php?cat=$cat&sortby=formalname&order=".('formalname' == $sortby ? ! $order : 1)."'>$mname</a></th><th><a href='modules.php?cat=$cat&sortby=moduleauthor&order=".('moduleauthor' == $sortby ? ! $order : 1)."'>$mauth</a></th><th><a href='modules.php?cat=$cat&sortby=installdate&order=".('installdate' == $sortby ? ! $order : 0)."'>$inon</a></th></thead></tr>");
        addnav('', "modules.php?cat=$cat&sortby=active&order=".('active' == $sortby ? ! $order : 1));
        addnav('', "modules.php?cat=$cat&sortby=formalname&order=".('formalname' == $sortby ? ! $order : 1));
        addnav('', "modules.php?cat=$cat&sortby=moduleauthor&order=".('moduleauthor' == $sortby ? ! $order : 1));
        addnav('', "modules.php?cat=$cat&sortby=installdate&order=".('installdate' == $sortby ? $order : 0));
    ...
    else
    {
        ...
        addnav('', "modules.php?op=mass&cat=$cat");
        rawoutput("<table class='ui small very compact selectable striped table'>", true);
        rawoutput("<thead><tr><th>&nbsp;</th><th>$ops</th><th><a href='modules.php?sorting=name&order=".('name' == $sorting ? ! $order : 0)."'>$mname</a></th><th><a href='modules.php?sorting=author&order=".('author' == $sorting ? ! $order : 0)."'>$mauth</a></th><th><a href='modules.php?sorting=category&order=".('category' == $sorting ? ! $order : 0)."'>$categ</a></th><th><a href='modules.php?sorting=shortname&order=".('shortname' == $sorting ? ! $order : 0)."'>$fname</a></th></tr></thead>");
        addnav('', 'modules.php?sorting=name&order='.('name' == $sorting ? ! $order : 0));
        addnav('', 'modules.php?sorting=author&order='.('author' == $sorting ? ! $order : 0));
        addnav('', 'modules.php?sorting=category&order='.('category' == $sorting ? ! $order : 0));
        addnav('', 'modules.php?sorting=shortname&order='.('shortname' == $sorting ? ! $order : 0));
        ...
        if (count($uninstmodules) > 0)
        {
            ...
            foreach ($uninstmodules as $key => $shortname)
            {
                    ...
                    false === strpos($file, $shortname.'_uninstall'))
                {
                    ...
                    $temp = array_merge($invalidmodule, ['name' => $shortname.'.php '.appoencode(translate_inline('(`$Invalid Module! Contact Author or check file!`0)'))]);
                }
                else
                {
            ...
            for ($a = 0; $a < count($moduleinfo); $a++)
            {
                ...
                if (isset($moduleinfo[$i]['invalid']) && true === $moduleinfo[$i]['invalid'])
                {
                    rawoutput('<td></td><td>');
                ...
                if (isset($moduleinfo[$i]['requires']) && count($moduleinfo[$i]['requires']))
                {
                    ...
                    foreach ($moduleinfo[$i]['requires'] as $key => $val)
                    {
                        ...
                        if (module_check_requirements([$key => $val]))
                        {
                            output_notl('`@');
                        }
                        else
