	...
	switch($hookname) {
		case "modifyuserview":
			...
			while ($row = db_fetch_assoc($result)){
				...
				$args['user'][$varname] = sprintf("Oro %s, Gemas %s, Ladungen %s [ <a href='user.php?op=edit&userid={$args['user']['acctid']}&deleteitem={$row['invid']}'>L�schen</a> ]", $row['sellvaluegold'], $row['sellvaluegems'], $row['charges']);
