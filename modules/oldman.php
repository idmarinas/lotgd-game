	...
	if ($bet<=0){
		...
		if ($guess!==0 || $try >= 1){
			if ($guess==$session['user']['specialmisc']){
				if ($try == 1) {
					output("`@\"`#INCREDIBLE!!!!`@\" the old man shouts, \"`#You guessed the number in only `^one try`#! Well, congratulations to you, and I am thoroughly impressed! It is almost as if you read my mind.`@\" He looks at you suspiciously and thinks about trying to make off with your winnings, but remembers your seemingly psychic abilities and hands over the `^%s`@ gold that he owes you.", $bet);
				} else {
					output("`@\"`#AAAH!!!!`@\" the old man shouts, \"`#You guessed the number in only %s tries!  It was `^%s`#!!  Well, congratulations to you, I think I'll just be going now... `@\" he says as he heads for the underbrush.`n`nA swift blow from your `^%s`@ knocks him unconscious.`n`nYou help yourself to his coinpurse, retrieving the `^%s`@ gold that he owes you.", $try, $session['user']['specialmisc'], $session['user']['weapon'], $bet);
				}
				...
				if ($try>=6&&($guess>=0&&$guess<=100)){
					output("`@The old man chuckles.  \"`#The number was `^%s`#,`@\" he says.  You, being the honorable citizen that you are, give the man the `^%s`@ gold that you owe him, ready to be away from here.", $session['user']['specialmisc'], $bet);
					$session['user']['specialinc']="";
					$session['user']['specialmisc']="";
					$session['user']['gold']-=$bet;
...
function oldman_runevent($type)
{
	...
	if ($op=="" || $op=="search"){
		...
		output("He beckons you over to talk with you.   He looks harmless enough, but you have heard tales of the evil creatures which lurk in this forest disguised as normal people.`n`n");
		addnav("Old Man");
		...
		case 2:
			...
			if ($session['user']['charm'] > 0) {
				...
				output("`@As you approach, he pulls out hist Ugly Stick, whacks you on the nose, then gasps as his stick `%loses one`@ charm point.`n`n");
		...
		case 4:
			// The betting game
			output("`@\"`#Would you like to play a little guessing game?`@\", he asks.  Knowing his sort, you know he will probably insist on a small wager if you do.`n`n");
		...
		case 5:
			...
			case 1:
				$session['user']['specialinc']="";
				output("`@When you reach him, he mutter, \"`#Didn't y' own mother teach you never t' talk t' strangers?`@\"  The old necromancer cackles and pulls out a black wand, waving it quickly over your head.`n`n");
				...
				output("`^That trecherous old man searches your body and takes all of your gold.`n");
			...
			case 3:
				$session['user']['specialinc']="";
				output("`@When you reach him, he mutters, \"`#Aye, me %s, come a wee bit closer.  That's it, just a bit CLOSER!`@\"`n`nAs the old necromancer screams that last word, he pulls out a black wand and your body twists in agony as if molten fire has replaced your blood.`n`nYour vision goes dark and you feel the hand of `\$Ramius`@ closing around your heart.  Just as you are sure you will die, the pain stops as quickly as it began.`n`nYou climb to your feet, shaking and weak.  The old necromancer is nowhere to be seen.`n`n", ($session['user']['sex']?"lass":"lad"));
				output("`@You feel you will never be quite the %s you were before.`n`n", ($session['user']['sex']?"woman":"man"));
				...
				addnews("%s came home from the forest, a bit less the " . ($session['user']['sex']?"woman":"man") . " than " . ($session['user']['sex']?"she":"he") . " was before.",$session['user']['name']);
				break;
			case 4:
			case 5:
			case 6:
			...
			case 15:
				$session['user']['specialinc']="";
				output("`@You are almost in front of him, when you hear a crashing sound coming from your right.`n`nYou turn to look and see one of bodyguards from the Inn walking through the woods heading your direction.  You turn back to the old man, but he seems to have vanished.`n`nOh well, you'll never know what he wanted now.");
				break;
			}
			break;
		}
		...
		if ($what1 == 0) {
			...
			output("`@No good deed goes unpunished!  You feel like a better person for helping the old man.");
		...
		output("`@Not a big deal, he should be able to find his way back to town on his own, he made his way out here, didn't he?  A wolf howls in the distance to your left, and a few seconds later one howls somewhere closer to your right.  Yep, he should be fine.");
		if (is_module_active('alignment')) {
			align("-1");
		}
	} elseif ($op == "nogame") {
		// Penny-pincher :)
		$session['user']['specialinc']="";
		output("`@Afraid to part with your precious precious money, you decline the old man his game.`n`nThere wasn't much point to it anyhow, as you certainly would have won.`n`nYep, definitely not afraid of the old man, nope.");
	} elseif ($op == "game") {
		if ($session['user']['gold'] <= 0) {
			$session['user']['specialinc']="";
			output("`@The old man reaches out with his stick and pokes your coin purse.  \"`#Empty?!?!  How can you bet with no money??`@\" he shouts.  With that, he turns with a HARUMPH, and disappears into the underbrush.");
		} else {
		...
		if ($session['user']['gems'] <= 0) {
			$session['user']['specialinc']="";
			output("`@You reach into your pack to find that you have no gems.  The old man looks at you expectantly.`n`nWhen he sees you have no gems, he starts to frown.   Sensing trouble, you turn to flee toward the forest.`n`nFrom behind you, you hear an evil laugh and feel a sharp pain in your back!`n`n");
			...
			$session['user']['specialinc']="";
			output("`@Feeling sorry for the old man, you reach into your pack and extract a gem.`nHe snatches it eagerly from your hand and ");
			$session['user']['gems']--;
			switch (e_rand(1,6)) {
			case 1:
			case 2:
			case 3:
				output("`@cackles with glee.  He turns to you and says, \"`#Since y' made such a fine bargain w' me, me %s, I'll be puttin' in a good word for y' with m' ol' friend `\$Ramius`#.`@\"`n`n", ($session['user']['sex']?"deary":"lad"));
			...
			case 4:
			case 5:
				output("`@cackles with glee.  He turns to you and says, \"`#Since y' made such a fine bargain w' me, me %s, I'll be teachin' y' a bit of m' art!`@\"`n`n", ($session['user']['sex']?"deary":"lad"));
				output("`@The necromancer pulls out a black wand, raps you thrice on the head, and runs off into the forest.`n`n");
				...
				$session['user']['specialty'] = 1;
				require_once("lib/increment_specialty.php");
				increment_specialty("`$");
				$session['user']['specialty'] = $oldspecialty;
				break;
			case 6:
				output("`@runs off into the forest.`n`nThat greedy old man stole your gem!");
			}
		}
	} elseif ($op == "keepgem") {
		$session['user']['specialinc']="";
		output("`@Not wanting to part with one of your precious gems, you turn around and march back toward the forest.`n`nFrom behind you, you hear an evil laugh and feel a sharp pain in your back!`n`n");
