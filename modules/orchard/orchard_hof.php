...
function orchard_hof() {
	...
	if ($totalpages>1){
		for($i = 0; $i < $totalpages; $i++) {
			...
			if ($maxpage==$minpage) addnav(array("Page %s (%s)", $j, $minpage), "runmodule.php?module=orchard&op=hof&subop=$j");
			else addnav(array("Page %s (%s-%s)", $j, $minpage, $maxpage), "runmodule.php?module=orchard&op=hof&subop=$j");
		}
	}
	$name = translate_inline("Name");
	$tree = translate_inline("Best Tree");
	$none = translate_inline("No Trees Found");
	output("`c`b`@Greatest `qArborists`@ in the Land`b`c");
