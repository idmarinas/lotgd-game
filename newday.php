...
if ($dp < $dkills) {
	...
	if (! is_module_active('staminasystem')) output("`2Turns for today set to `^%s`2.`n",$turnsperday);
	...
	if (abs($spirits)>0)
	{
		if($resurrectionturns > 0) $gain = translate_inline("gain");
		else $gain= translate_inline("lose");

		if (is_module_active('staminasystem')) output('`2As a result, you `^%s some Stamina`2 for today!`n', $gain);
		else
		{
	...
	if ($resurrection != "true") $session['user']['playerfights'] = $dailypvpfights;
