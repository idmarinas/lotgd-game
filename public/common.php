...
if (isset($session['lasthit'], $session['loggedin'])
    && strtotime(
         '-'
         . LotgdSetting::getSetting('LOGINTIMEOUT', 900)
         . ' seconds'
       ) > $session['lasthit']
    && $session['lasthit'] > 0
    && $session['loggedin']) {
...
if (!isset($nokeeprestore[LotgdRequest::getServer('SCRIPT_NAME')])
    || !$nokeeprestore[LotgdRequest::getServer(
	'SCRIPT_NAME'
  )]) {
...
if ($url != $site && $uri && $site) {
	$url = sprintf(
	  '%s://%s/%s',
	  LotgdRequest::getServer('REQUEST_SCHEME'),
	  $url,
	  LotgdRequest::getServer('REQUEST_URI')
	);

	$refererRepository = Doctrine::getRepository(Referers::class);
	$referers = $refererRepository->findOneByUri($uri);
	$referers = $referers ?: new Referers();
