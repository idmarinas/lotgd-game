...
$basetext = [
    ...
    'desc' => [
        ...
        ["You approach, and he whirls around, pointing a pitchfork in your general direction, \"`&Ach, sorry m'%s, I dinnae hear ya' comin' up on me, an' I thoht fer sure ye were %s`&; he what been tryin' to improve on his dwarf tossin' skills. ", translate_inline($session['user']['sex'] ? 'lass' : 'lad'), getsetting('barkeep', '`tCedrik')],
...
if (0 == $confirm)
{
    if ($session['user']['hashorse'] > 0)
    {
        addnav(['%s', color_sanitize($name)]);
        tlschema($schemas['offer']);
        output($texts['offer'], $repaygold, $repaygems, $lcname);
        tlschema();
        addnav(['Sell %s`0', $lcname], 'stables.php?op=sellmount');

        if (getsetting('allowfeed', 0) && 0 == $session['user']['fedmount'])
        {
            addnav(['Feed %s`0 (`^%s`0 gold)', $lcname, $grubprice],
                    'stables.php?op=feed');
        }
    }
    ...
    for ($i = 0; $i < $number; $i++)
    {
        ...
        {
            addnav(['%s', $row['mountcategory']]);
            $category = $row['mountcategory'];
        }

        if ($row['mountdkcost'] <= $session['user']['dragonkills'])
        {
            addnav(['Examine %s`0', $row['mountname']], "stables.php?op=examine&id={$row['mountid']}");
        }
    }
}
