...
else if ($act == 'attack')
{
	...
	$options['type'] = "pvp";
	$failedattack = false;
    if ($badguy === false)  { $failedattack = true; }
    else
    {
		$battle=true;
    ...
    if ($failedattack)
    {
        if (httpget('inn') > '') { addnav('Return to Listing', 'inn.php?op=bartender&act=listupstairs'); }
        else { addnav('Return to Listing', 'pvp.php'); }
	}
}
...
if ($battle)
{
    ...
    if ($victory)
    {
		...
		$op = '';
		httpset('op', $op);
        if ($killedin == $iname) { addnav("Return to the inn","inn.php"); }
        else { villagenav(); }
