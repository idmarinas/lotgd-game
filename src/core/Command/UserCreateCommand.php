...
final class UserCreateCommand extends Command
{
	...
	protected const SU_MEGAUSER = SU_MEGAUSER
	                              | SU_EDIT_MOUNTS
	                              | SU_EDIT_CREATURES
	                              | SU_EDIT_PETITIONS
	                              | SU_EDIT_COMMENTS
			  ...
			  new Assert\Callback(function ($login, ExecutionContextInterface $context): void {
				  ...
				  if ($exists) {
					  $context->addViolation(
						$this->translator->trans('user.create.question.login.exists', [], self::TEXT_DOMAIN)
					  );
				  }
			  }),
			]);
		...
		$question->setValidator(function ($value) {
			$errors = $this->validator->validate((string)$value, [
			  ...
			  new Assert\Callback(function ($email, ExecutionContextInterface $context): void {
				  ...
				  if ($exists && $email) {
					  $context->addViolation(
						$this->translator->trans('user.create.question.email.exists', [], self::TEXT_DOMAIN)
					  );
				  }
			  }),
			]);
	...
	{
		/** @var UserRepository $superusers */
		$superusers = (bool)$this->getAccountRepository()->getSuperuserCountWithPermit(SU_MEGAUSER);

		if ($superusers) { //-- If exist one super user not allow create more
			return false;
		}
