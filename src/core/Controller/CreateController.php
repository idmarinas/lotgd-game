...
class CreateController extends AbstractController
{
  ...
  $accountRepo = $this->getDoctrine()->getRepository('LotgdCore:User');
		...
		if (!$account) {
			$this->addFlash(
			  'warning',
			  $this->translator->trans('validating.pass.paragraph', [], $params['textDomain'])
			);
  ...
  $accountRepo = $this->getDoctrine()->getRepository('LotgdCore:User');
		...
		if (!$account) {
			$this->addFlash(
			  'warning',
			  $this->translator->trans('validating.email.paragraph.fail', [], $params['textDomain'])
			);
		...
		if ($account->getReplaceemail()) {
			...
			$this->log->debug(
			  'Email change request validated by link from ' . $oldEmail . ' to ' . $replaceemail,
			  $account->getAcctid(),
			  $account->getAcctid(),
			  'Email'
			);
	...
	public function forgot (array $params, Request $request): Response
	{
		...
		if ('' !== $charname && '0' !== $charname) {
			...
			if (!$account instanceof User) {
				$this->addFlash(
				  'warning',
				  $this->translator->trans('forgot.account.notFound', [], $params['textDomain'])
				);
			...
			if ('' === trim($account->getEmailaddress()) || '0' === trim($account->getEmailaddress())) {
				$this->addFlash(
				  'warning',
				  $this->translator->trans('forgot.account.noEmail', [], $params['textDomain'])
				);
			...
			$language = ($account->getPrefs()['language'] ?? '') ?:
			  $this->settings->getSetting('defaultlanguage', 'en');
			...
			$msg = $this->translator->trans('forgotpassword.body', [
			  ...
			  'gameurl'      => 'https://' . ($request->getServer('SERVER_NAME') . '/' . $request->getServer(
					'SCRIPT_NAME'
				  )),
	...
	public function index (array $params, Request $request): Response
	{
		if ('create' == $request->query->get('op')) {
			...
			if ($this->censor->filter($shortname) !== $shortname) {
				...
				$this->addFlash(
				  'error',
				  $this->translator->trans('create.account.badLanguage', [], $params['textDomain'])
				);
			}
			...
			if (1 == $this->settings->getSetting('blockdupeemail', 0)
			    && 1 == $this->settings->getSetting(
				'requireemail',
				0
			  )) {
				...
				if (!empty($result)) {
					$this->addFlash(
					  'error',
					  $this->translator->trans('create.account.email.duplicate', [], $params['textDomain'])
					);
					$blockaccount = true;
				}
			}
			...
			if (strlen($pass1) <= 3) {
				$this->addFlash(
				  'error',
				  $this->translator->trans('create.account.password.length', [], $params['textDomain'])
				);
				$blockaccount = true;
			}
			...
			if ($pass1 !== $pass2) {
				$this->addFlash(
				  'error',
				  $this->translator->trans('create.account.password.notIdentical', [], $params['textDomain'])
				);
				$blockaccount = true;
			}
			...
			if (strlen($shortname) < 3) {
				$this->addFlash(
				  'error',
				  $this->translator->trans('create.account.name.minLength', [], $params['textDomain'])
				);
				$blockaccount = true;
			}
			...
			if (strlen($shortname) > 25) {
				$this->addFlash(
				  'error',
				  $this->translator->trans('create.account.name.maxLength', [], $params['textDomain'])
				);
				$blockaccount = true;
			}
			...
			if (1 == (int)$this->settings->getSetting('requireemail', 0) && !$this->validator->isMail($email)) {
				$this->addFlash(
				  'error',
				  $this->translator->trans('create.account.email.incorrect', [], $params['textDomain'])
				);
				$blockaccount = true;
			}
			...
			if (null !== $result) {
				...
				$this->addFlash(
				  'error',
				  $this->translator->trans('create.account.name.duplicate', [], $params['textDomain'])
				);
			}
			...
			if (!$blockaccount) {
				...
				try {
					...
					$characterEntity
					  ->setPlayername((string)$shortname)
					  ->setSex($sex)
					...
					if ('' != $emailverification) {
						...
						$msg = $this->translator->trans('verificationmail.body', [
						  ...
						  'gameurl'      => 'https://' . ($request->getServer('SERVER_NAME')
						                                  . '/'
						...
						$this->addFlash(
						  ...
						  $this->translator->trans(
							'create.account.emailVerification',
							['email' => $email],
							$params['textDomain']
						  )
						);
					...
					$this->addFlash(
					  'error',
					  $this->translator->trans('create.account.error', [], $params['textDomain'])
					);
				}
			}
		}
