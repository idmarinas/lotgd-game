...
class MotdRepository extends ServiceEntityRepository
{
	...
	public function getLastMotd (): ?array
	{
		...
		try {
			$result = $qb
			  ->select('u', 'c.name as motdauthorname')
	...
	public function appendPollResults (array $motd, $userId): array
	{
		$q = $this->_em->createQuery(
		  'SELECT COUNT(p.resultid) AS votes, p.choice
		...
		if (0 !== $userId) {
			$q = $this->_em->createQuery(
			  'SELECT p.choice FROM Lotgd\Core\Entity\PollResults p WHERE p.motditem = :motditem AND p.account = :acct'
			);
			$q->setParameters([
	...
	{
		$q = $this->_em->createQuery(
		  'SELECT YEAR(u.motddate) AS year, MONTH(u.motddate) AS month, u.motddate AS date, COUNT(MONTH(u.motddate)) AS count
	...
	public function getLastMotdDate (): ?DateTime
	{
		...
		return $result !== null ? $result->getMotddate() : null;
	}
}
