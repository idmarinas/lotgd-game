...
trait Navigation
{
	...
	public function createLink ($label, $options): ?string
	{
		...
		if ($options['translate'] ?? false) {
			$label = $this->translator->trans(
				$label,
				$options['params'] ?? [],
				$options['textDomain'] ?? 'navigation_app',
				$options['locale'] ?? null
			);
		} else {
	 ...
	 * @param string|array|null $template       You can change the template for your own if you need it at a specific
	 *                                          time For render a block use array ['block_name', 'path/to/template']
	...
	public function showPagination (
		...
		$link = preg_replace(
			'/(?:[?&]c=[[:digit:]]+)|(?:[?&]page=[[:digit:]]+)|(?:[?&]commentPage=[[:digit:]]+)/i',
			'',
			$link
		);
